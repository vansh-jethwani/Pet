...
const BREEDING_TYPES  = new Set(["breeding_like","breeding_match","breeding_message","adoption_applied","adoption_approved","hosting_request","hosting_confirmed"]);
const ORDERS_TYPES    = new Set(["store_order","marketplace_interest","insurance_expiry"]);

function applyTab(ns: AppNotification[], tab: Tab): AppNotification[] {
  switch (tab) {
    case "unread":    return ns.filter(n => !n.read);
    case "breeding":  return ns.filter(n => BREEDING_TYPES.has(n.type));
    case "vet":       return ns.filter(n => n.type.startsWith("vet"));
    case "community": return ns.filter(n => n.type.startsWith("community"));
    case "orders":    return ns.filter(n => ORDERS_TYPES.has(n.type));
    default:          return ns;
  }
}
...
function NRow({ n, onRead, onUnread, onDelete, onAction }: {
  ...
  return (
    ...
    >
      {/* Unread dot */}
      {!n.read && (
        <div className={cn("absolute left-1.5 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full", c.dot)} />
      )}

      {/* Icon bubble */}
      <div className={cn(
        "w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-base",
        n.read ? "bg-gray-100" : c.light
      )}>
        {n.icon}
      </div>
      ...
      <div className="flex-1 min-w-0 pr-1">
        ...
        {n.actionLabel && n.actionUrl && (
          <button
            ...
            className={cn(
              "inline-flex items-center gap-1 mt-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all",
              "border hover:opacity-90 active:scale-95",
              c.text,
              n.read
                ? "border-gray-200 bg-gray-50 hover:bg-gray-100"
                : `border-current/20 ${c.light} hover:brightness-95`
            )}
          >
            {n.actionLabel} →
          </button>
      ...
      {/* Context menu — stop propagation so row click doesn't fire */}
        ...
        {menu && (
          <div className="absolute right-0 top-7 z-[60] w-44 bg-white rounded-xl border border-gray-100 shadow-2xl overflow-hidden">
            {n.read ? (
              <button
                type="button"
                onClick={() => { onUnread(n.id); setMenu(false); }}
              ...
              <button
                type="button"
                onClick={() => { onRead(n.id); setMenu(false); }}
                className="w-full flex items-center gap-2 px-3.5 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
              >
            ...
            {n.actionUrl && (
              <button
                type="button"
                onClick={() => { onAction(n); setMenu(false); }}
                className="w-full flex items-center gap-2 px-3.5 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
              >
                <ChevronDown className="w-3.5 h-3.5 text-orange-500 -rotate-90" />
                {n.actionLabel || "Open"}
              </button>
            )}
            ...
            <button
              type="button"
              onClick={() => { onDelete(n.id); setMenu(false); }}
...
.notif-slide-down { animation: notif-slide-down 0.2s cubic-bezier(.34,1.56,.64,1) both; }
...
export default function NotificationWidget(props: WidgetProps) {
  const {
    ...
    isOpen, onClose,
    onMarkRead, onMarkUnread, onMarkAllRead,
  ...
  // Derived lists — recalculated whenever notifications or tab change
  const filtered  = useMemo(() => applyTab(notifications, tab), [notifications, tab]);
  const readCount = useMemo(() => notifications.filter(n => n.read).length, [notifications]);

  // Tab badge counts — always derived from full notifications list
  const tabCounts = useMemo<Record<Tab, number>>(() => ({
    all:       notifications.length,
    unread:    notifications.filter(n => !n.read).length,
    breeding:  applyTab(notifications, "breeding").length,
    vet:       applyTab(notifications, "vet").length,
    community: applyTab(notifications, "community").length,
    orders:    applyTab(notifications, "orders").length,
  }), [notifications]);

  // Outside-click: use bubble phase (capture=false, the default).
  // The bell button wrapper in Header.tsx calls e.stopPropagation() which
  // prevents the mousedown from ever reaching this document listener —
  // so clicking the bell only runs toggleNotif, never onClose().
  // Clicks genuinely outside the panel bubble up freely and trigger onClose().
  // NOTE: capture=true would fire BEFORE stopPropagation, breaking the bell toggle.
  useEffect(() => {
    if (!isOpen) return;
  ...
  // Don't unmount — just hide with CSS so the socket stays alive
  ...
  return (
    <>
      ...
      {/* Mobile backdrop */}
      <div
        className="fixed inset-0 z-[45] sm:hidden bg-black/30"
        onClick={onClose}
      />
      ...
      <div
        ref={panelRef}
        className="notif-slide-down fixed z-50 right-4 top-[73px] w-[calc(100vw-32px)] sm:w-[400px] max-h-[80vh] sm:max-h-[580px] bg-white rounded-2xl border border-gray-200 shadow-2xl flex flex-col overflow-hidden"
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              {readCount > 0 && (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 text-[11px] font-bold text-gray-400 hover:text-red-500 transition-colors",
                    unreadCount === 0 && "ml-auto"
                  )}
                >
                  <Trash2 className="w-3 h-3" /> Clear read ({readCount})
                </button>
              )}
            </div>
          )}
          ...
          {TABS.map(t => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span className={cn(
                    "text-[10px] font-black px-1 rounded-full",
                    tab === t.id
        ...
        <div className="flex-1 overflow-y-auto notif-hide-scroll">
          {loading && notifications.length === 0 ? (
            ...
            <>
              ...
              {/* Load more — only show in "all" tab when more pages exist */}
              {hasMore && tab === "all" && (
